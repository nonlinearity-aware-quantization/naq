...
def attention_forward_attn_score(self, x):
    B, N, C = x.shape
...
def attention_forward(self, x):
    ...
    attn = self.softmax(input = attn, quantized_input = quantized_attn)
...
def window_attention_attn_score(self, x, mask = None):
    B_, N, C = x.shape
...
def get_net(name, naq_config = None):
    ...
    for name, module in net.named_modules():
        if isinstance(module, Attention):
            ...
            module.attn_score = MethodType(attention_forward_attn_score, module)
            module.forward = MethodType(attention_forward, module)
        if isinstance(module, WindowAttention):
            setattr(module, "matmul1", MatMul())
            setattr(module, "matmul2", MatMul())
            module.attn_score = MethodType(window_attention_attn_score, module)
