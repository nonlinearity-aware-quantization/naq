...
class OPTAttention(nn.Module):
    ...
    attention_mask: Optional[torch.Tensor] = None):
        ...
        return bsz, tgt_len, src_len, value_states, attn_weights
    ...
    def forward(
        ...
        if self.naq_config is not None and 'awsm_quantization' in self.naq_config:
            ...
            _, _, _, _, quantized_attn = self.attn_score(quantized_hidden_states, quantized_key_value_states, quantized_past_key_value, attention_mask)
        else:
            quantized_attn = None

        # upcast to fp32 if the weights are in fp16. Please see https://github.com/huggingface/transformers/pull/17437
        # if attn_weights.dtype == torch.float16:
        #     attn_weights = self.softmax(attn_weights, quantized_attn, dtype=torch.float32).to(torch.float16)
        # else:
        attn_weights = self.softmax(attn_weights, quantized_attn)
