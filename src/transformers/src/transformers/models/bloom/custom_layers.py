...
class Linear_GELU_SignedBitMaskMultiplicationThresholdPrediction(Linear_GELU):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        proportion_weight_zero = torch.sum(torch.where(self.weight == 0, 1, 0), dim=-1, keepdim=True)/weight_sign_values.numel()
        output_calc_sparsity = torch.ones(input.shape[0], input.shape[1], 1, device=input.device) @ proportion_weight_zero.t()
        ...
        reverse_quantized_output_sign_values_and_gelu = bloom_gelu_forward(output_sign_values_rev_quantized)
        ...
        output = bloom_gelu_forward(linear_output)
        ...
        collect_prediction_stats(predicted, actual, threshold, single_value_prediction, num_bits_to_calculate, output_calc_sparsity)
...
def collect_prediction_stats(predicted, actual, threshold = 0, single_value_prediction = 0, num_bits_to_calculate = 0, output_calc_sparsity = None):
    ...
    if output_calc_sparsity is not None:
        average_non_predicted_sparsity = torch.sum(torch.where(predicted == 0, 1, 0) * output_calc_sparsity) / torch.sum(torch.where(predicted == 0, 1, 0))
